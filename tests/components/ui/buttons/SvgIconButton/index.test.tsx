...
describe('SvgIconButton', () => {
  ...
  describe('Input/Output', () => {
    describe('標準ボタン', () => {
      beforeEach(() => {
        result = render(<SvgIconButton icon={{ variant: SVG_ICON_VARIANT.CHECK }} onClick={() => handleClick()} />)
      })
    ...
    describe('title を指定した場合', () => {
      beforeEach(() => {
        result = render(
          <SvgIconButton icon={{ variant: SVG_ICON_VARIANT.CHECK }} title="決定する" onClick={() => handleClick()} />
        )
      })
  ...
  describe('Display', () => {
    describe('ベーススタイル', () => {
      ...
      test('svgIconButton クラスが適用されている', () => {
        const button = result.container.querySelector('button')
        expect(button?.className).toContain('svgIconButton')
      })
    })
    ...
    describe('非活性ボタン', () => {
      beforeEach(() => {
        result = render(
          <SvgIconButton icon={{ variant: SVG_ICON_VARIANT.CHECK }} isDisabled onClick={() => handleClick()} />
        )
      })
      ...
      test('非活性スタイルが適用されている', () => {
        const button = result.container.querySelector('button')
        expect(button?.className).toContain('svgIconButton__disabled')
      })
    })
    ...
    describe('内部構造', () => {
      ...
      test('icon クラスが SvgIcon に引き渡されている', () => {
        const icon = result.container.querySelector('i[class*="svgIconButton_icon"]')
        expect(icon).not.toBe(null)
      })
    })
    ...
    describe('icon にカスタムクラス名を指定した場合', () => {
      ...
      test('svgIconButton_icon クラスも維持されている', () => {
        const icon = result.container.querySelector('i')
        expect(icon?.className).toContain('svgIconButton_icon')
      })
    ...
    describe('リンクボタンのスタイル', () => {
      beforeEach(() => {
        result = render(
          <MemoryRouter>
            <SvgIconButton icon={{ variant: SVG_ICON_VARIANT.ARROW_FORWARD }} url="/internal/path" />
          </MemoryRouter>
        )
      })

      test('リンク要素にも SvgIconButton のスタイルが適用されている', () => {
        const link = result.container.querySelector('a')
        expect(link?.className).toContain('svgIconButton')
      })
    })
  })
